...
"""
import re, os, secrets, ast
import yaml


META = {"required", "one_of", "header_required", "document", "shorthand",
        "qualified_values_when"}

# Crockford base32, lowercase — i, l, o and u are absent, which is what keeps a
# minted code out of reach of YAML's implicit resolvers: `false` needs an `l`,
# `true` a `u`, `null` an `l`, and nothing numeric begins with a letter.
...
# Protocol paths this file reads. Hand-maintained, and deliberately visible: it
# is the difference between "protocol is missing declarations.factor" and a
# KeyError two hundred lines into a loop. `self_check` verifies each resolves;
# nothing verifies the list is complete, which is why the derivable half —
# every key subscripted on a document — is checked separately from the source.
NEEDS = [
    ...
    ("declarations", "frame", "required"), ("declarations", "frame", "role"),
    ("declarations", "frame", "set_by"), ("declarations", "frame", "one_of"),
    ("declarations", "factor", "required"), ("declarations", "factor", "set_by"),
    ...
    ("composition", "entry", "mode"), ("composition", "entry", "required"),
    ("relations", "edge", "required"),
]

# Which document variable each pass subscripts, and the shape whose `required`
# list has to promise those keys.
...
def check_pred(pred, where, allowed, fail, forbidden=frozenset(), noun="frame"):
    """A predicate is `{ <id>: [value, ...] }` or `{ <id>: { facet: [...] } }`.

    `allowed` is the id set a predicate in this position may name. `forbidden`
    exists for the message rather than the verdict: a universe predicate that
    names a factor is wrong for a reason worth saying, and "unknown frame"
...
def _package_source():
    """Every module in this package, concatenated.

    `_document_keys` finds each pass by name and reads the keys it subscripts.
    Reading only this file would mean a future split of the checking passes
    silently narrows the convention to whichever module kept `self_check` —
    a check that quietly stops checking. Reading the package keeps the
    convention as wide as the code.
    """
...
def _document_keys(source):
    """Keys subscripted on a document variable, per pass, read off this file.

    A conditional subscript would be a false positive, but the convention is
    that optional access goes through `.get()` — so anything subscripted is
    something this file assumes present, which is exactly the claim to check.
    """
...
def self_check(p, source=None):
    ...
    # the paths this file reads must resolve
    for path in NEEDS:
        node = p
        for step in path:
            node = node.get(step) if isinstance(node, dict) else None
            if node is None:
                fail.append(f"protocol is missing `{'.'.join(path)}`, which this checker reads")
                break

    for path, shape in _walk_shapes(p):
        where = ".".join(path) or "<root>"
        # `required` on a shape carrying a `document` names the document's keys;
        # everywhere else it names the shape's own. Stated as a rule so that
        # universe and guidance need no special case.
        if "required" in shape:
            target = shape["document"] if isinstance(shape.get("document"), dict) else shape
            ...
            if not isinstance(req, list):
                ...
                if bad:
                    fail.append(f"{where}.required names {bad}, which "
        ...
        if "one_of" in shape:
            ...
            if not isinstance(choices, list):
                ...
                if bad:
                    fail.append(f"{where}.one_of names {bad}, which the shape does not declare")
        if "header_required" in shape:
            own = path[-1] if path else ""
            header = (shape.get("document") or {}).get(own)
            if not isinstance(header, dict):
                fail.append(f"{where}.header_required: no `document.{own}` to resolve against")
            else:
                bad = [k for k in shape["header_required"] if k not in header]
                if bad:
                    fail.append(f"{where}.header_required names {bad}, which "
                                f"`document.{own}` does not declare")

    # document types and the shapes that declare a document are one set
    declared = {k for k, v in d.items() if isinstance(v, dict) and "document" in v}
    kinds = set((d.get("document_types") or {}).get("kinds") or [])
    if kinds - declared:
        fail.append(f"document_types.kinds names {sorted(kinds - declared)}, "
                    f"which declare no `document` shape")
    if declared - kinds:
        fail.append(f"{sorted(declared - kinds)} declares a `document` shape but is not in "
                    f"document_types.kinds")

    # a key this file subscripts on a document is a key the protocol must require
    if source is None:
        source = _package_source()
    for shape, (fname, var, keys) in _document_keys(source).items():
        req = set((d.get(shape) or {}).get("required") or [])
        for k in sorted(keys - req):
            fail.append(f"{fname}() subscripts {var}['{k}'] but `{shape}.required` does not "
                        f"promise it — subscript it and require it, or read it with .get()")
    return fail


def missing_preconditions(d, doc, shape, name):
    """Required document and header keys, checked before anything reads them.

    Everything below subscripts these, so a missing one is a precondition
    failure rather than one finding among many — recording it and carrying on
    would reach the subscript and raise instead of reporting.
    """
    fail = [f"document: missing required '{k}'"
            for k in d[shape]["required"] if k not in doc]
    if not fail:
        fail = [f"header: missing required '{k}'"
                for k in d[shape]["header_required"] if k not in doc[shape]]
    if fail:
        print()
        for f_ in fail:
            print(f"  FAIL  {f_}")
...
def check(p, u, name):
    ...
    for label, used, allowed in [
        ...
        *([("universe overview", set(u["universe"]["overview"]),
            set(doc_shape["universe"]["overview"]))]
          if isinstance(u["universe"].get("overview"), dict) else []),
        ("element", set().union(*[set(e) for e in u["elements"]]),
            (sanc(d["element"]) - {"<ordering-frame-id>"}) | {ordering}),
        ("artifact", set().union(*[set(a) for a in u["artifacts"]]), sanc(d["artifact"])),
        ("frame", set().union(*[set(f) for f in u["frames"]]), sanc(d["frame"])),
        *([("factor", set().union(*[set(k) for k in factors]), sanc(d["factor"]))]
          if factors else []),
        ("relation_kind", set().union(*[set(r) for r in u["relation_kinds"]]),
    ...
    # frame role and set_by
    roles = alts(d["frame"]["role"])
    set_bys = alts(d["frame"]["set_by"])
    for f in u["frames"]:
        if f.get("role") not in roles:
            fail.append(f"frame {f['id']}: role '{f.get('role')}' not in {sorted(roles)}")
        if f.get("set_by") not in set_bys:
            fail.append(f"frame {f['id']}: set_by '{f.get('set_by')}' not in {sorted(set_bys)}")
    ord_frames = [f["id"] for f in u["frames"] if f.get("role") == "ordering"]
    if ord_frames != [ordering]:
        fail.append(f"frames with role ordering {ord_frames} != header ordering_frame ['{ordering}']")

    # factor set_by — its own loop, because a factor declares no role
    factor_set_bys = alts(d["factor"]["set_by"])
    for k in factors:
        if k.get("set_by") not in factor_set_bys:
            fail.append(f"factor {k.get('id')}: set_by '{k.get('set_by')}' "
                        f"not in {sorted(factor_set_bys)}")

    # exactly one of values, facets, source — declared on both shapes
    for kind, items in [("frame", u["frames"]), ("factor", factors)]:
        choices = d[kind]["one_of"]
        for i in items:
            present = [c for c in choices if c in i]
            if len(present) != 1:
                fail.append(f"{kind} {i.get('id')}: declares {sorted(present) or 'none'} "
                            f"— exactly one of {choices} is required")

    # required fields — a precondition for the same reason document keys are:
    # the checks below subscript them, so recording a miss and carrying on
    # reaches the subscript and raises instead of reporting.
    required_fail = []
    for key, items in [("element", u["elements"]), ("artifact", u["artifacts"]),
                       ("frame", u["frames"]), ("factor", factors),
            ...
            if types == {"dict"}:
                for v in f["values"]:
                    if set(v) != {"id", "question"}:
                        fail.append(f"{kind} {f['id']}: qualified value keys "
                                    f"{sorted(v)} != [id, question]")
            else:
                fail.append(f"{kind} {f['id']}: mixed value types {sorted(types)}")

    # predicates name frames, never factors — a factor is wired into nothing
    ...
    # enums and value shapes
    for e in u["elements"]:
        if not re.fullmatch(r"singleton|per-[a-z-]+", str(e["cardinality"])):
            fail.append(f"element {e['id']}: cardinality '{e['cardinality']}' malformed")
        if "closable" in e and not isinstance(e["closable"], bool):
            fail.append(f"element {e['id']}: closable not boolean")

    # codes — a handle, not a key: stable, unordered, and unique universe-wide,
    # since the kind letter carries the set
    minted = {}
    for kind, items in [("element", u["elements"]), ("artifact", u["artifacts"]),
                        ("frame", u["frames"]), ("factor", factors)]:
        ...
        for i in items:
            ...
            if not isinstance(c, str):
                fail.append(f"{kind} {i.get('id')}: code {c!r} loaded as "
                            f"{type(c).__name__}, not a string")
                continue
            if not shape.fullmatch(c):
                fail.append(f"{kind} {i.get('id')}: code '{c}' does not match "
                            f"'{CODE_PREFIX[kind]}{CODE_TAIL}'")
            if c in minted:
                fail.append(f"{kind} {i.get('id')}: code '{c}' already carried by "
    ...
    for kind, items in [("artifact", u["artifacts"]), ("frame", u["frames"]),
                        ("factor", factors), ("relation_kind", u["relation_kinds"])]:
    ...
    # composition entries name an element; the shorthand is the element itself
        ...
        for lst in a["composition"]:
            for it in (a["composition"].get(lst) or []):
                if not isinstance(it, dict):
                    ...
                    continue
                if req_entry - set(it):
                    fail.append(f"{a['id']}/{lst}: composition entry missing required "
                                f"{sorted(req_entry - set(it))}")
                    continue
                composed.add(it["element"])
    if composed - ids:
        fail.append(f"composition refs unknown elements: {sorted(composed - ids)}")
    # relation edge shape — required fields first, since everything downstream
    # reads them; an edge that fails here is not carried into those passes
    ...
    for r in u["relations"]:
        bad = set(r) - edge_keys
        if bad:
            fail.append(f"relation {r.get('from')}->{r.get('to')}: "
    ...
    dangling = [(r["from"], r["to"]) for r in edges
    ...
    # every edge on a cycle declares legality — ordered kinds only, since an
    # unordered kind is a symmetric statement rather than a traversal
    ...
    for r in edges:
        if r["kind"] not in ordered:
            continue
        if (r["to"] == r["from"] or reaches(r["to"], r["from"])) \
                and "legality" not in r:
            fail.append(f"relation {r['from']}->{r['to']} ({r['kind']}) "
    ...
    print()
    print(f"  counts: {len(u['elements'])} elements, {len(u['artifacts'])} artifacts, "
...
def check_guidance(p, g, universes, name):
    ...
    for label, used, allowed in [
        ...
        ("guidance_kind", set().union(*[set(k) for k in g["guidance_kinds"]]),
    ...
    if u["universe"].get("conforms_to") != g["guidance"].get("conforms_to"):
        fail.append(f"conforms_to '{g['guidance'].get('conforms_to')}' does not match "
    ...
    total = 0
    for section, valid in [("elements", element_ids), ("artifacts", artifact_ids),
                           ("factors", factor_ids)]:
        for key, entries in (g.get(section) or {}).items():
            if key not in valid:
                fail.append(f"{section}.{key}: resolves to no declared id in '{guides}'")
            for i, it in enumerate(entries or []):
                ...
                if "when" in it:
                    # a frame or factor the guided universe declares — both id
                    # sets come from that one file
                    check_pred(it["when"], f"{where}.when",
                               frame_ids | factor_ids, fail, noun="frame or factor")

    unused = sources - {it["source"]
                        for section in ("elements", "artifacts", "factors")
                        for entries in (g.get(section) or {}).values()
                        for it in (entries or []) if isinstance(it, dict) and "source" in it}
    if unused:
    ...
    print()
    print(f"  counts: {total} entries over {len(g.get('elements') or {})} elements, "
...
def mint(kind, count, doc_paths=()):
    """Fresh codes for one set, colliding with nothing the given documents declare.

    Random rather than sequential on purpose: a code that can be guessed from
    position is a code someone will read an order into.
    """
    taken = set()
