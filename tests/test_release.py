...
def project(tmp_path, monkeypatch):
    for name in ("pyproject.toml", ".claude-plugin/plugin.json", "uv.lock",
                 "spec/knowledge-bus-protocol.yaml"):
    ...
    plugin_file = tmp_path / ".claude-plugin/plugin.json"
...
def finish_notes(project):
    ...
    path.write_text(path.read_text().replace("TODO: Describe the user-facing changes.", "Add automatic discovery."))


def test_prepare_synchronizes_without_changing_protocol(project):
    protocol = (project / "spec/knowledge-bus-protocol.yaml").read_bytes()
    release.prepare("0.6.0", project)
    assert release.metadata(project) == ("0.6.0", "0.6.0", "kbp/0.5")
    assert (project / "spec/knowledge-bus-protocol.yaml").read_bytes() == protocol
...
@pytest.mark.parametrize("version", ["v0.6.0", "0.6", "01.6.0", "0.6.0-rc1", "0.5.0", "0.4.0"])
...
def test_prepare_rejects_existing_tag(project, monkeypatch):
    monkeypatch.setattr(release, "git",
                        lambda *args, **kwargs: "v0.6.0" if args[0] == "tag" else "")
...
def test_metadata_rejects_mismatched_manifest_and_protocol_notes(project):
    release.prepare("0.6.0", project)
    finish_notes(project)
    path = project / ".claude-plugin/plugin.json"
...
def test_tag_refuses_feature_branch(project, monkeypatch):
    monkeypatch.setattr(release, "git",
                        lambda *args, **kwargs: "feature" if args[0] == "branch" else "")
    with pytest.raises(ValueError, match="from main"):
        release.tag(project)


def test_tag_requires_synced_main_and_only_creates_local_annotated_tag(project, monkeypatch):
    calls = []
    remote = ["behind"]
    def git(*args, **kwargs):
        if args[0] == "branch":
    ...
    remote[0] = "abc"
    release.tag(project)
    assert calls[-1] == ("git", "tag", "-a", "v0.5.0", "abc", "-m", "Knowledge Bus 0.5.0")
...
def test_verify_tag_checks_name_annotation_commit_and_ancestry(project, monkeypatch):
    ...
    assert calls[-1] == ("git", "merge-base", "--is-ancestor", "abc", "refs/remotes/origin/main")
...
def test_version_without_user_directory(tmp_path):
    ...
    assert result.stdout == f"Knowledge Bus {release.metadata(ROOT)[0]}\nBundled protocol: kbp/0.5\n"
