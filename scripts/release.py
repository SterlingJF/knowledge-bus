...
def version_tuple(value):
    if not VERSION.fullmatch(value):
        raise ValueError("Use a release version X.Y.Z without a v prefix or prerelease suffix.")
...
def metadata(root=ROOT):
    ...
    protocol = yaml.safe_load((root / "spec/knowledge-bus-protocol.yaml").read_text())["protocol"]
    version_tuple(project["version"])
    return project["version"], plugin["version"], f"{protocol['id']}/{protocol['version']}"


def require_clean(root=ROOT):
    if git("status", "--porcelain", root=root):
        raise ValueError("Commit or set aside working-tree changes first; nothing was changed.")


def require_unused(version, root=ROOT):
    if git("tag", "--list", f"v{version}", root=root):
        raise ValueError(f"Tag v{version} already exists; release tags must not be reused.")


def release_notes(version, root=ROOT):
    text = (root / "CHANGELOG.md").read_text()
    matches = list(re.finditer(r"^## " + re.escape(version) + r"\s*$", text, re.M))
    if len(matches) != 1:
    ...
    if re.search(r"\b(TODO|TBD)\b", body, re.I) or not re.search(r"^- \S", body, re.M):
        raise ValueError("Finish the release changelog: include changes and remove TODO/TBD placeholders.")
    return body
...
def prepare(version, root=ROOT):
    ...
    paths = [root / p for p in ("pyproject.toml", ".claude-plugin/plugin.json", "uv.lock", "CHANGELOG.md")]
    originals = {p: p.read_bytes() for p in paths}
    try:
        project = paths[0].read_text()
        # Replace only the project version, not dependency or tool versions.
        section = re.search(r"(?ms)^\[project\]\s*\n(.*?)(?=^\[|\Z)", project)
        body, count = re.subn(r'(?m)^version\s*=\s*"[^"]+"', f'version = "{version}"', section[1])
        if count != 1:
            raise ValueError("Expected one static project.version in pyproject.toml.")
        paths[0].write_text(project[:section.start(1)] + body + project[section.end(1):])
        ...
        raise
    print(f"Prepared {version}, unstaged. Finish CHANGELOG.md, then run just release-check.")
...
def build_check(version, root=ROOT, output=None):
    with tempfile.TemporaryDirectory(prefix="knowledge-bus-release-") as temporary:
        base = Path(temporary)
        dist = base / "dist"
        run("uv", "build", "--out-dir", str(dist), root=root)
        wheel, = dist.glob("*.whl")
        ...
        displayed = run(str(python), "-I", "-m", "kbp_conform.cli", "--version", root=base, capture=True)
        protocol = metadata(root)[2]
        if displayed.strip() != f"Knowledge Bus {version}\nBundled protocol: {protocol}":
        ...
        # Rebuild from the sdist to prove it includes the protocol and build inputs.
        rebuilt = base / "rebuilt"
        run("uv", "build", str(sdist), "--wheel", "--out-dir", str(rebuilt), root=base)
        rebuilt_wheel, = rebuilt.glob("*.whl")
        run("uv", "pip", "install", "--reinstall", "--python", str(python), str(rebuilt_wheel), root=base)
        run(str(python), "-I", "-m", "kbp_conform.cli", "--self-check", root=base)
        if output:
...
def check(root=ROOT, output=None):
    ...
    run("uv", "run", "--locked", "kbp", "--validate",
        str(root / "spec/knowledge-bus-protocol.yaml"), str(root / "universes"), root=root)
...
def tag(root=ROOT):
    ...
    # Fetch only; never push or change the checkout.
    ...
    run("git", "tag", "-a", f"v{version}", head, "-m", f"Knowledge Bus {version}", root=root)
    print(f"Created local tag v{version}. Publish with: git push origin refs/tags/v{version}")
...
def verify_tag(name, root=ROOT):
    ...
    run("git", "merge-base", "--is-ancestor", commit, "refs/remotes/origin/main", root=root)
    return notes
