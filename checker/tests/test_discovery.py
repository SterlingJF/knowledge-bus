...
def knowledge_bus_dir(folder, *, populated=True):
    ...
    if populated:
        for name in ("universe.kbp.yaml", "type-guidance.kbp.yaml"):
            shutil.copyfile(
                ROOT / "universes/product-development" / name, metadata / name
            )
    return metadata
...
def test_nested_empty_knowledge_bus_dir_does_not_fall_back(
...
def test_explicit_target_overrides_current_knowledge_bus_dir(
    tmp_path, monkeypatch, metadata_target
):
...
def test_missing_knowledge_bus_dir_never_initializes_or_discovers_sibling(
...
def test_generic_directory_scan_skips_nested_knowledge_bus_directories(tmp_path):
    ...
    shutil.copyfile(ROOT / "protocol/conformance/pass/minimal.kbp.yaml", document)
...
def test_knowledge_bus_dir_loads_guidance_but_not_ingestion_outputs(
    tmp_path, monkeypatch, capsys
):
...
def test_explicit_protocol_uses_current_knowledge_bus_dir(tmp_path, monkeypatch):
    metadata = knowledge_bus_dir(tmp_path)
    monkeypatch.chdir(tmp_path)
    protocol = ROOT / "protocol" / cli.PROTOCOL_NAME
...
@pytest.mark.parametrize("relative", [".", "checker", "checker/src/kbp_conform"])
...
def test_packaged_protocol_is_independent_of_current_directory(tmp_path, monkeypatch):
    ...
    shutil.copyfile(ROOT / "protocol" / cli.PROTOCOL_NAME, protocol)
